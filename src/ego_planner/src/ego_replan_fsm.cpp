...
namespace ego_planner
{
  ...
  void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    ...
    const double goal_z = std::max(0.1, msg->pose.position.z);
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, goal_z);

#if EGO_ENABLE_GOAL_REACHABILITY_PROJECTION
    if (planner_manager_ && planner_manager_->grid_map_)
    {
      auto map = planner_manager_->grid_map_;
      bool in_map = map->isInMap(end_wp);
      int occ = in_map ? map->getInflateOccupancy(end_wp) : 1;

      if (!in_map || occ == 1)
      {
        const double res = std::max(0.05, map->getResolution());
        const double max_radius = 5.0;
        Eigen::Vector3d best = end_wp;
        double best_dist = std::numeric_limits<double>::max();
        bool found = false;

        std::vector<double> z_candidates;
        z_candidates.push_back(end_wp(2));
        for (int layer = 1; layer <= 4; ++layer)
        {
          z_candidates.push_back(end_wp(2) + layer * res);
          z_candidates.push_back(end_wp(2) - layer * res);
        }

        for (double radius = res; radius <= max_radius; radius += res)
        {
          int samples = std::max(16, static_cast<int>(2.0 * M_PI * radius / res));
          for (double zc : z_candidates)
          {
            for (int index = 0; index < samples; ++index)
            {
              double theta = 2.0 * M_PI * static_cast<double>(index) / static_cast<double>(samples);
              Eigen::Vector3d cand(
                  end_wp(0) + radius * std::cos(theta),
                  end_wp(1) + radius * std::sin(theta),
                  zc);

              if (!map->isInMap(cand))
                continue;
              if (map->getInflateOccupancy(cand) != 0)
                continue;

              double dist = (cand - end_wp).norm();
              if (dist < best_dist)
              {
                best = cand;
                best_dist = dist;
        ...
        if (found)
        {
          ROS_WARN("[EGO] Goal projected from (%.2f, %.2f, %.2f) to (%.2f, %.2f, %.2f)",
                   end_wp(0), end_wp(1), end_wp(2), best(0), best(1), best(2));
          end_wp = best;
        }
        else
        {
          ROS_WARN_THROTTLE(1.0, "[EGO] Goal blocked and no nearby free voxel found, skip this goal frame.");
          return;
        }
      }
    }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      ...
      {
#if EGO_ENABLE_MANUAL_TARGET_DIRECT_START
        if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
        {
