...
int Pose3DTool::processMouseEvent(ViewportMouseEvent& event)
{
  ...
  else if (event.type == QEvent::MouseMove && event.left())
  {
    ...
    if (state_ == Height)
    {
      ...
      pos_.z -= dz / z_scale;
      arrow_->setPosition(pos_);
      for (int index = 0; index < static_cast<int>(arrow_array.size()); index++)
        delete arrow_array[index];
      arrow_array.clear();
      int count = ceil(fabs(initz - pos_.z) / z_interval);
      for (int index = 0; index < count; index++)
      {
        Arrow* arrow_marker = new Arrow(scene_manager_, NULL, 0.5f, 0.1f, 0.0f, 0.1f);
        arrow_marker->setColor(0.0f, 1.0f, 0.0f, 1.0f);
        arrow_marker->getSceneNode()->setVisible(true);
        Ogre::Vector3 arr_pos = pos_;
        arr_pos.z = initz - ((initz - pos_.z > 0) ? 1 : -1) * index * z_interval;
        arrow_marker->setPosition(arr_pos);
        arrow_marker->setOrientation(Ogre::Quaternion(Ogre::Radian(prevangle), Ogre::Vector3::UNIT_Z) * orient_x);
        arrow_array.push_back(arrow_marker);
      }
      flags |= Render;
    }
  }
