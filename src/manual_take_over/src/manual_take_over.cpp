...
int main(int argc, char **argv)
{
  ...
  string mandatory_stop_topic("/mandatory_stop_to_planner");
  string joystick_topic("/joystick_from_bridge");
  string cmd_topic("/position_cmd");
  nh.param("mandatory_stop_topic", mandatory_stop_topic, mandatory_stop_topic);
  nh.param("joystick_topic", joystick_topic, joystick_topic);
  nh.param("cmd_topic", cmd_topic, cmd_topic);

  mandatory_stop_pub_ = nh.advertise<std_msgs::Empty>(mandatory_stop_topic, 10);
  cmd_pub_ = nh.advertise<quadrotor_msgs::PositionCommand>(cmd_topic, 10);
  ros::Subscriber joy_sub = nh.subscribe<sensor_msgs::Joy>(joystick_topic, 10, joy_sub_cb);
  ros::Subscriber cmd_sub = nh.subscribe<quadrotor_msgs::PositionCommand>(cmd_topic, 100, position_cmd_sub_cb);
